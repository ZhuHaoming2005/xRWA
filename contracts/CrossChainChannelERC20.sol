...
/**
 * @title Cross-Chain Channel Contract for ERC20
 ...
 */
contract CrossChainChannelERC20 is ReentrancyGuard {
    ...
    // Events
    event ChannelOpened(bytes32 indexed channelId, address indexed participant, address indexed counterparty, address token, uint256 deposit);
    ...
    function openChannel(
        ...
        emit ChannelOpened(channelId, msg.sender, counterparty, token, amount);
        return channelId;
    }
    ...
    function updateChannel(UpdateData calldata update) external isOpen(update.channelId) {
        ...
        uint256 newLocked;
        if (channel.isBuyer) {
            require(update.buyerAssets.length == 1, "Invalid buyer assets format for ERC20");
            newLocked = update.buyerAssets[0];
        } else {
            require(update.sellerAssets.length == 1, "Invalid seller assets format for ERC20");
    ...
    function unlock(bytes32 channelId, bytes32 preimage) external nonReentrant isOpen(channelId) {
        ...
        IERC20(channel.token).safeTransfer(channel.counterparty, amount);
    ...
    function closeChannel(bytes32 channelId) external nonReentrant isOpen(channelId) {
        ...
        if (remaining > 0) {
            IERC20(channel.token).safeTransfer(channel.participant, remaining);
        }
